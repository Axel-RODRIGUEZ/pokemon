...
class DisplayBattle(Display):
    ...
    def __init__(self, screen: Surface, fonts: tuple[font.Font, font.Font], user_sprite: Surface, wild_sprite: Surface, user: User, wild_pokemon: Pokemon):
    ...
    def update(self, buttons: list = []):
        ...
        for pokemon in self.__user.pokedex:
            if pokemon["name"]["fr"] == self.__user.main:
                self.draw_text(f"Niveau 12", (0,0,0), (400,620) ,self._screen) #Changer niveau
                self.draw_text(self.__user.main, (0,0,0), (420,640), self._screen)
